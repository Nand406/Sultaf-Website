...
<div class="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-6 space-y-5">
        ...
        <div>
            ...
            <textarea name="deskripsi" rows="3" placeholder="Deskripsi singkat menu..."
                      class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-sultaf-maroon">{{ old('deskripsi', $menu->deskripsi ?? '') }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Harga Jual (Rp)</label>
                <input type="number" name="harga_makanan" value="{{ old('harga_makanan', $menu->harga_makanan ?? '') }}"
                       min="0" step="500" placeholder="75000" required
                       class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-sultaf-maroon">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Harga Modal / HPP (Rp)</label>
                <input type="number" name="harga_modal" value="{{ old('harga_modal', $menu->harga_modal ?? 0) }}"
                       min="0" step="500" placeholder="35000"
                       class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-sultaf-maroon">
                <p class="text-xs text-gray-400 mt-1">Dipakai untuk hitung keuntungan di dashboard Owner. Boleh dikosongkan (0) dulu.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Level Pedas (0–3)</label>
        ...
        <div class="flex items-center gap-6 pt-2">
            ...
            <label class="flex items-center gap-2 cursor-pointer">
                ...
                <span class="text-sm text-gray-700">Tandai sebagai habis</span>
            </label>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-6 h-fit">
        ...
        @if (!empty($menu) && $menu->foto_makanan)
            ...
            <p class="text-xs text-gray-400 mb-3">Upload foto baru untuk mengganti.</p>
        @endif

        <label class="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-gray-200 rounded-xl py-8 cursor-pointer hover:border-sultaf-maroon transition-colors">
            <svg class="w-6 h-6 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 16V4M7 9l5-5 5 5M5 20h14"/></svg>
            <span id="fileLabel" class="text-sm text-gray-500 font-medium text-center px-2">Klik untuk pilih foto</span>
...
<div class="flex items-center gap-3 mt-6">
    ...
    <a href="{{ route('admin.menu.index') }}" class="text-sm font-medium text-gray-500 hover:text-gray-700 px-4">Batal</a>
</div>
