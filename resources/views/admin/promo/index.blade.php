    ...
    <div class="flex flex-col lg:flex-row gap-6 items-start">
        ...
        <div class="w-full lg:w-96 shrink-0">
            <div class="staff-card p-6">
                <h3 class="font-semibold text-sultaf-ink mb-4">Pesan Baru</h3>

                <form method="POST" action="{{ route('admin.promo.store') }}" class="space-y-4">
                    @csrf

                    <div>
                        <label class="block text-sm font-semibold text-sultaf-ink mb-1.5">Judul</label>
                    ...
                    <div>
                        ...
                        <textarea name="pesan" rows="4" required placeholder="Tulis pesan promosi di sini..."
                                  class="w-full border border-sultaf-border rounded-xl px-4 py-2.5 text-sm bg-white focus:outline-none focus:border-sultaf-maroon"></textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-sultaf-ink mb-1.5">Kirim Kepada</label>
        ...
        <div class="flex-1 min-w-0 w-full">
            <h3 class="font-semibold text-sultaf-ink mb-4">Riwayat Pesan Terkirim</h3>
                ...
                @forelse ($pesanTerkirim as $pesan)
                    <div class="staff-card p-5">
                        <div class="flex items-start justify-between mb-2">
                            <div>
                                <h4 class="font-semibold text-sultaf-ink">{{ $pesan->judul }}</h4>
                                <p class="text-xs text-sultaf-muted mt-0.5">
                            ...
                            <form method="POST" action="{{ route('admin.promo.destroy', $pesan) }}"
                                ...
                                @csrf @method('DELETE')
                                <button class="text-xs text-sultaf-muted hover:text-sultaf-danger">Hapus</button>
                            </form>
                        </div>
                        <p class="text-sm text-sultaf-ink/80 mb-3">{{ $pesan->pesan }}</p>
