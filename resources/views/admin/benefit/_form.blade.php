...
<div class="staff-card p-6 max-w-xl space-y-5">

    <div>
        <label class="block text-sm font-semibold text-sultaf-ink mb-1.5">Nama Promosi</label>
    ...
    <div>
        ...
        <textarea name="deskripsi" rows="2" placeholder="Deskripsi singkat promosi..."
                  class="w-full border border-sultaf-border rounded-xl px-4 py-2.5 text-sm bg-white focus:outline-none focus:border-sultaf-maroon">{{ old('deskripsi', $benefit->deskripsi ?? '') }}</textarea>
    </div>

    <div>
        <label class="block text-sm font-semibold text-sultaf-ink mb-1.5">Minimal Poin Dibutuhkan</label>
        <input type="number" name="poin_dibutuhkan" value="{{ old('poin_dibutuhkan', $benefit->poin_dibutuhkan ?? '') }}"
               min="1" placeholder="500" required
               class="w-full border border-sultaf-border rounded-xl px-4 py-2.5 text-sm bg-white focus:outline-none focus:border-sultaf-maroon">
        <p class="text-xs text-sultaf-muted mt-1">Member yang mengumpulkan poin sejumlah ini atau lebih bisa klaim promosi.</p>
    </div>

    <div>
        <label class="block text-sm font-semibold text-sultaf-ink mb-1.5">Tipe Reward</label>
        <div class="grid grid-cols-2 gap-3">
            <label class="cursor-pointer">
                ...
                <div class="text-center py-2.5 rounded-xl border border-sultaf-border text-sm font-medium text-sultaf-ink
    ...
    <div id="diskonField" class="{{ old('tipe', $benefit->tipe ?? 'diskon') === 'gratis_menu' ? 'hidden' : '' }}">
        <label class="block text-sm font-semibold text-sultaf-ink mb-1.5">Nilai Diskon (%)</label>
    ...
    <div id="menuField" class="{{ old('tipe', $benefit->tipe ?? 'diskon') === 'gratis_menu' ? '' : 'hidden' }}">
        <label class="block text-sm font-semibold text-sultaf-ink mb-1.5">Pilih Menu Gratis</label>
        <select name="menu_id"
                class="w-full border border-sultaf-border rounded-xl px-4 py-2.5 text-sm bg-white focus:outline-none focus:border-sultaf-maroon">
    ...
    <label class="flex items-center gap-2 cursor-pointer pt-2">
        ...
        <span class="text-sm text-sultaf-ink">Aktifkan promosi ini</span>
    </label>
</div>
...
<div class="flex items-center gap-3 mt-6">
    ...
    <a href="{{ route('admin.benefit.index') }}" class="text-sm font-medium text-sultaf-muted hover:text-sultaf-ink px-4">Batal</a>
</div>
