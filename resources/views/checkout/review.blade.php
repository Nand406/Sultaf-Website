    ...
    <div class="px-5 lg:px-0 pt-6 pb-6">
        ...
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-3">
                <a href="{{ route('menu.index') }}" class="text-sultaf-ink hover:text-sultaf-maroon">
        ...
        @if (session('error'))
            <div class="mb-4 text-sm text-sultaf-danger bg-sultaf-danger-soft rounded-xl px-4 py-3">{{ session('error') }}</div>
        @endif
        @if (session('success'))
            <div class="mb-4 text-sm text-sultaf-success bg-sultaf-success-soft rounded-xl px-4 py-3">✓ {{ session('success') }}</div>
        @endif
        ...
        <div class="flex flex-col lg:flex-row gap-8 items-start">
            ...
            <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-4 p-4">
                            ...
                            <div class="text-right shrink-0">
                                ...
                                <form method="POST" action="{{ route('cart.remove', $item['menu']) }}">
                                    @csrf @method('DELETE')
                                    <button class="text-xs text-sultaf-muted hover:text-sultaf-danger mt-1">{{ __('Remove') }}</button>
                                </form>
                            </div>
                        </div>
                    @empty
                ...
                @if ($items->isNotEmpty())
                    <div class="hidden lg:flex mt-6 rounded-2xl bg-sultaf-maroon-dark bg-sultaf-pattern-dark overflow-hidden items-end relative h-44">
            ...
            @if ($items->isNotEmpty())
                <div class="w-full lg:w-80 xl:w-96 shrink-0 lg:sticky lg:top-24">
                    <div class="bg-white rounded-2xl border border-sultaf-border/70 p-5 space-y-3">
